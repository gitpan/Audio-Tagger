...
Create an C<Audio::Tagger::File> object given a file name.
...
sub new {
	...
	return Audio::Tagger::File::_tagger_file_file_new($filename);
}
...
sub save {
	...
	return $self -> _tagger_file_file_save();
}
...
sub DESTROY {
	...
	$self -> _tagger_file_file_destroy();
}
...
sub title {
	...
	(defined $title)						?
		$self -> _tagger_file_tag_set_str(0, $title)		:
		$self -> _tagger_file_tag_get_str(0);
}
...
sub artist {
	...
	(defined $artist)						?
		$self -> _tagger_file_tag_set_str(1, $artist)	:
		$self -> _tagger_file_tag_get_str(1);
}
...
sub album {
	...
	(defined $album)						?
		$self -> _tagger_file_tag_set_str(2, $album)		:
		$self -> _tagger_file_tag_get_str(2);
}
...
sub comment {
	...
	(defined $comment)						?
		$self -> _tagger_file_tag_set_str(3, $comment)	:
		$self -> _tagger_file_tag_get_str(3);
}
...
sub genre {
	...
	(defined $genre)						?
		$self -> _tagger_file_tag_set_str(4, $genre)		:
		$self -> _tagger_file_tag_get_str(4);
}
...
sub year {
	...
	(defined $year)							?
		$self -> _tagger_file_tag_set_num(0, $year)		:
		$self -> _tagger_file_tag_get_num(0);
}
